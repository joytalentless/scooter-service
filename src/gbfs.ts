...
import { boltOsloScooterPrise, boltFredrikstadScooterPrise, boltLillestromScooterPrise, defaultScooterPrice, limeScooterPrice } from './utils/constants'
...
app.get('/:provider/:feed', async (req, res): Promise<void> => {
  const {
...
function mapFeed<T extends keyof typeof Provider, S extends keyof typeof Feed>(provider: T, feed: S, feedResponse: string): GBFSBase {
  switch (feed) {
    case Feed.gbfs:
...
function mapDiscoveryFeed<T extends keyof typeof Provider>(provider: T, feedResponse: string): GBFS {
...
function mapSystemInformationFeed<T extends keyof typeof Provider>(provider: T, feedResponse: string): SystemInformation {
  const {
    last_updated,
...
function getVehicleTypesFeed<T extends keyof typeof Provider>(provider: T): VehicleTypes {
  const codespace = getCodespace(provider)
...
function mapVehicleTypesFeed<T extends keyof typeof Provider>(provider: T, feedResponse: string): VehicleTypes {
  ...
  return {
    ...
    version: '2.1',
    data: {
      vehicle_types: vehicleTypes.data.vehicle_types.map(vehicleType => {
        return {
...
function mapFreeBikeStatusFeed<T extends keyof typeof Provider>(provider: T, feedResponse: string): FreeBikeStatus {
  ...
  return {
    ...
    data: {
        bikes: freeBikeStatus.data.bikes.map((bike: any) => ({
            ...
            vehicle_type_id: `${codespace}:VehicleType:${bike.vehicle_type_id || 'Scooter'}`,
            current_range_meters: bike.current_range_meters || 0,
            pricing_plan_id: `${codespace}:PricingPlan:${bike.pricing_plan_id || 'Basic'}`,
...
function getSystemPricingPlansFeed<T extends keyof typeof Provider>(provider: T): SystemPricingPlans {
  const codespace = getCodespace(provider)
...
function getSystemPricing<T extends keyof typeof Provider>(provider: T): ScooterPrice {
  switch(provider) {
...
function getFeedUrl<T extends keyof typeof Provider, S extends keyof typeof Feed>(provider: T, feed: S): string {
  switch (provider) {
    case Provider.voioslo:
      return functions.config().voi.url.oslo.replace('free_bike_status', feed)
    case Provider.voitrondheim:
      return functions.config().voi.url.trondheim.replace('free_bike_status', feed)
    case Provider.limeoslo:
      return functions.config().lime.url.oslo.replace('free_bike_status', feed)
    case Provider.boltoslo:
      return functions.config().bolt.url.oslo.replace('free_bike_status', feed)
    case Provider.boltfredrikstad:
      return functions.config().bolt.url.fredrikstad.replace('free_bike_status', feed)
    case Provider.boltlillestrom:
      return functions.config().bolt.url.lillestrom.replace('free_bike_status', feed)
    default:
      throw new Error("Unknown provider")
  }
}

async function getBearerToken<T extends keyof typeof Provider>(provider: T): Promise<string> {
  switch(provider) {
    case Provider.voioslo:
    case Provider.voitrondheim:
...
async function getFeed<T extends keyof typeof Provider>(provider: T, feedUrl: string, bearerToken: string): Promise<string> {
  switch(provider) {
    case Provider.voioslo:
    case Provider.voitrondheim:
...
async function getVoiFeed(feedUrl: string, bearerToken: string): Promise<string> {
...
async function getLimeFeed(feedUrl: string, bearerToken: string): Promise<string> {
  const response: request.Response = await request
    .get(feedUrl)
    .set('Authorization', `Bearer ${bearerToken}`)
  return response.text
...
async function getBoltFeed(feedUrl: string, bearerToken: string): Promise<string> {
  const response: request.Response = await request
    .get(feedUrl)
    .set('Authorization', `Bearer ${bearerToken}`)
